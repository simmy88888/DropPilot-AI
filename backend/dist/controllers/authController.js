...
const signup = async (req, res) => {
    ...
    try {
        ...
        const stmt = db_1.default.prepare('INSERT INTO users (email, password_hash, name) VALUES (?, ?, ?)');
...
const getMe = async (req, res) => {
    try {
        const user = db_1.default.prepare('SELECT id, email, name, role FROM users WHERE id = ?').get(req.user.id);
        if (!user) {
